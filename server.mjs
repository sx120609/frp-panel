...
function renderFrpcConfig(client) {
  ...
  const lines = [
    `serverAddr = ${quote(s.serverAddr.replace(/^https?:\/\//, '').replace(/:\d+$/, ''))}`,
    `serverPort = ${s.serverPort}`,
    '',
    '# Keep retrying after a transient network loss instead of exiting on the first failed login.',
    'loginFailExit = false',
    '',
    'auth.method = "token"',
    `auth.token = ${quote(s.authToken)}`,
    '',
    'transport.dialServerTimeout = 10',
    'transport.dialServerKeepalive = 30',
    '# Use TCP-mux keepalive instead of the application heartbeat watchdog.',
    'transport.tcpMuxKeepaliveInterval = 15',
...
function windowsInstallScript(client) {
  ...
  return [
    ...
    `Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Description "frpc (${name}) managed by frp-panel" -Force | Out-Null`,
